...
#include "dev/ConstructBooleanMaps.h"
#include "ModelMap.h"
...
#include "dev/AnnotationConditions.h"
...
private:
    ...
    LUT lut_;
    // return true if the morse graph contains a morse set with
    // walls along all space variables
    // if true the morse set has an annotation "good"
    bool validateMorseGraph ( MorseGraph * mg_in ) const;
    // Check if a Morse Set satisfies all condition to be "valid"
...
//    if ( condition4 ) {
//        annotation . push_back ( CONDITION4STRING );
//    }
    return annotation;
}
